...
public class ThoughtMongoService implements ThoughtService {
    ...
    @Override
    public ResponseEntity<Thought> findById(String id) {
        try {
            Optional<Thought> optional = _thoughtRepository.findById(id);
            Thought thoughtFound = optional.get();
            thoughtFound.assignStringId();
            return ResponseEntity.ok(thoughtFound);
        } catch (NoSuchElementException e) {
            throw new NotFoundException("Thought with id " + id + " doesn't exist");
        } catch (Exception e) {
